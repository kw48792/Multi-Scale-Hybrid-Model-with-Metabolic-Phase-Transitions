...
theta.add('vmax3f_0', value=0.850073394869049, min=0, max=2*1E2)
...
theta.add('vmax16f_0', value=0.09, min=0, max=1*1E2)
theta.add('vmax16r_0', value=0.003, min=0, max=0.1*1E2)
theta.add('vmax18f_0', value=0.000, min=0, max=1*1E2)
theta.add('vmax18r_0', value=0.00, min=0, max=0.1*1E2)
theta.add('vmax19_0', value=6.601569291220244e-10, min=0, max=0.1*1E2)
theta.add('vmax21_0', value=0.03, min=0, max=1*1E2)
theta.add('vmax23_0', value=0.015, min=0, max=1*1E2)
theta.add('vmax25_0', value=0.015, min=0, max=1*1E2)
...
theta.add('vmax30_0', value=0.035, min=0, max=1*1E5)
...
##Late Exponential Phase
theta.add('vmax2_1', value=0.07701946222030127, min=0, max=4*1E2)
...
theta.add('vmax5f_1', value=0.012, min=0, max=1*1E2)
theta.add('vmax5r_1', value=0.002, min=0, max=0.4*1E2)
theta.add('vmax7_1', value=0.002844105585730894, min=0, max=1*1E2)
theta.add('vmax16f_1', value=0.000, min=0, max=1*1E2)
theta.add('vmax16r_1', value=0.013, min=0, max=0.1*1E2)
...
theta.add('vmax21_1', value=0.000, min=0, max=1*1E2)
theta.add('vmax23_1', value=0.000, min=0, max=1*1E2)
theta.add('vmax25_1', value=0.000, min=0, max=1*1E2)
...
theta.add('vmax30_1', value=0.023, min=0, max=1*1E5)
...
##Stational Phase
theta.add('vmax2_2', value=0.02701946222030127, min=0, max=4*1E2)
...
theta.add('vmax16r_2', value=0.0068, min=0, max=0.1*1E2)
theta.add('vmax18f_2', value=0.009, min=0, max=1*1E2)
theta.add('vmax18r_2', value=0.000, min=0, max=0.1*1E2)
theta.add('vmax19_2', value=0.003109839922081024, min=0, max=0.1*1E2)
theta.add('vmax21_2', value=0.002, min=0, max=1*1E2)
theta.add('vmax23_2', value=0.000, min=0, max=1*1E2)
theta.add('vmax25_2', value=0.0015, min=0, max=1*1E2)
...
theta.add('vmax30_2', value=0.0035, min=0, max=1*1E5)
...
theta.add('K_mEGLU_2', value=6, min=0, max=0.3*1E2)
...
##Decline Phase
theta.add('vmax2_3', value=0.0550705865745511, min=0, max=4*1E2)
theta.add('vmax3f_3', value=0.0003747313147894893, min=0, max=2*1E2)
...
theta.add('vmax16r_3', value=0.0012, min=0, max=0.1*1E2)
theta.add('vmax18f_3', value=0.009, min=0, max=1*1E2)
theta.add('vmax18r_3', value=0.002, min=0, max=0.1*1E2)
theta.add('vmax19_3', value=0.003262152166469912, min=0, max=0.1*1E2)
theta.add('vmax21_3', value=0.0042, min=0, max=1*1E2)
theta.add('vmax23_3', value=0.0042, min=0, max=1*1E2)
theta.add('vmax25_3', value=0.0029, min=0, max=1*1E2)
theta.add('vmax27f_3', value=0.02, min=0, max=0.2*1E2)
theta.add('vmax27r_3', value=0.01, min=0, max=0.1*1E2)
theta.add('vmax28_3', value=0.0002, min=0, max=0.1*1E2)
theta.add('vmax30_3', value=-0.0055, min=-0.04 * 600, max=0.04 * 600)
...
with open('params_A1.pkl', 'wb') as f:
    pickle.dump(theta, f)
...
for i, key in enumerate(extracellular_keys.keys()):
    ...
    ax.set_ylim(bottom=-0.05 * np.max(meas_vals))
    ax.grid(True)
...
plt.tight_layout()
plt.savefig('Prediction_A1.png')
plt.show()


#%% minimizing WAPE of one metabolite only
def objective_function_single(params, target_key, determ=True):
    total_wape = 0.0
    batch_count = 0
    for b in ['tigr15_1']:
        wape_dict, overall_wape, t, x = calc_wape_for_batch(b, params, mode_var=0, determ=determ)
...
def objective_print_single(params):
    val = objective_function_single(params, target_key='EASP', determ=True)
    eval_counter['n'] += 1
    print(f"Iteration {eval_counter['n']}: EASP error = {val}")
    return val
...
try:
    ...
    val = objective_function_single(res.params, target_key='EASP', determ=True)
    print(f"  -> finished Nelder-Mead, EASP value = {val}\n")
    best_res = res
    best_val = val
...
for b in ['tigr15_1']:
    ...
    x0 = np.concatenate((np.zeros(target_label_mapper['ANTI'] + 1), data_extra_cont[b].iloc[0, 1:-1].values))
    x0 = np.hstack((x0, np.array((data_extra_cont[b].iloc[0, -1] * biomass, 0, 0, data_extra_cont[b].iloc[0, -1], 0, 0))))  # Initial VCD and Biomass
    x = model_prediction_determ(t, x0, params_single, mode = 0, batch=b, t_eval=None, growth_prop=0, metabolite_prop=0)


    # Plot predicted vs measured for key extracellular metabolites for batch b

    extracellular_keys = ['EGLC','EGLN','EGLU','EASP','EALA','ELAC',
                          'ESER','ELEU','EILE','EVAL','ENH4','EANTI','VCD', 'VCD_0', 'VCD_1', 'VCD_2']
...
#%%
###Calculate and print WAPE for each batch
for b in ['tigr15_1']:
    wape_dict, overall_wape, t, x = calc_wape_for_batch(b, theta, mode_var=1)
    print(f"Batch: {b}")
    for key, wape in wape_dict.items():
        if np.isnan(wape):
            print(f"  WAPE for {key}: nan")
        else:
            print(f"  WAPE for {key}: {wape:.2f}%")
    if np.isnan(overall_wape):
        print("  Overall WAPE: nan\n")
    else:
...
#%%
###Data A1
data_A1 = pd.read_excel("A1.xlsx", sheet_name=None)
for key in data_A1.keys():
    print(key)

fig, axs = plt.subplots(3, 3, figsize=(15, 9))

for idx, i in enumerate(data_A1['Measurement'].columns[1:]):
    # Plot measurements as dots
    axs[idx // 3, idx % 3].plot(data_A1['Measurement']['Day'], data_A1['Measurement'][i], 'o', label='Measure', color = 'darkorange')
    
    # Plot predictions as solid lines
    axs[idx // 3, idx % 3].plot(data_A1['Measurement']['Day'], data_A1['Prediction'][i], '-', label='Prediction', color = 'royalblue')
    
    # Plot prediction intervals as shaded areas
    axs[idx // 3, idx % 3].fill_between(data_A1['Measurement']['Day'], 
                                        data_A1['Prediction_Upper'][i], 
                                        data_A1['Prediction_Lower'][i], 
                                        alpha=0.2, label='PI')
    
    axs[idx // 3, idx % 3].set_ylabel(i, fontsize=15)
    axs[idx // 3, idx % 3].set_xlabel('Time (day)', fontsize=15)
    axs[idx // 3, idx % 3].tick_params(axis='both', which='major', labelsize=12)
    axs[idx // 3, idx % 3].legend(loc='best')
    
    # Ensure y-axis range is larger than prediction upper
    max_y = max(data_A1['Prediction_Upper'][i].max(), data_A1['Measurement'][i].max())
    axs[idx // 3, idx % 3].set_ylim(0, max_y * 1.1)
    
    # Increase density of y-ticks
    axs[idx // 3, idx % 3].yaxis.set_major_locator(plt.MaxNLocator(nbins=8))

    # Add the index to the plots
    axs[idx // 3, idx % 3].text(0.05, 0.2, f'({chr(97+idx)})', horizontalalignment='center', verticalalignment='center', transform=axs[idx // 3, idx % 3].transAxes, fontsize=15, fontweight='bold', va='top')

plt.tight_layout()
plt.savefig('A1_prediction.png')
plt.show()

#%%
###Data A2
data_A2 = pd.read_excel("A2.xlsx", sheet_name=None)
for key in data_A2.keys():
    print(key)

fig, axs = plt.subplots(3, 3, figsize=(15, 9))

for idx, i in enumerate(data_A2['Measurement'].columns[1:]):
    # Plot measurements as dots
    axs[idx // 3, idx % 3].plot(data_A2['Measurement']['Day'], data_A2['Measurement'][i], 'o', label='Measure', color = 'darkorange')
    
    # Plot predictions as solid lines
    axs[idx // 3, idx % 3].plot(data_A2['Measurement']['Day'], data_A2['Prediction'][i], '-', label='Prediction', color = 'royalblue')
    
    # Plot prediction intervals as shaded areas
    axs[idx // 3, idx % 3].fill_between(data_A2['Measurement']['Day'], 
                                        data_A2['Prediction_Upper'][i], 
                                        data_A2['Prediction_Lower'][i], 
                                        alpha=0.2, label='PI')
    ...
    # Ensure y-axis range is larger than prediction upper
    max_y = max(data_A2['Prediction_Upper'][i].max(), data_A2['Measurement'][i].max())
    axs[idx // 3, idx % 3].set_ylim(0, max_y * 1.1)
...
plt.tight_layout()
plt.savefig('A2_prediction.png')
plt.show()


#%%
###Data FDA
data_FDA = pd.read_excel("FDA.xlsx", sheet_name=None)
for key in data_FDA.keys():
    print(key)

fig, axs = plt.subplots(2, 2, figsize=(10, 8))

for idx, i in enumerate(data_FDA['Measurement'].columns[1:]):
    # Plot measurements as dots
    axs[idx // 3, idx % 3].plot(data_FDA['Measurement']['Day'], data_FDA['Measurement'][i], 'o', label='Measure', color = 'darkorange')
    
    # Plot predictions as solid lines
    axs[idx // 3, idx % 3].plot(data_FDA['Measurement']['Day'], data_FDA['Prediction'][i], '-', label='Prediction', color = 'royalblue', linewidth=3)
    # Plot prediction intervals as shaded areas
    axs[idx // 3, idx % 3].fill_between(data_FDA['Measurement']['Day'], 
                                        data_FDA['Prediction_Upper'][i], 
                                        data_FDA['Prediction_Lower'][i], 
                                        alpha=0.2, label='PI')
    
    axs[idx // 3, idx % 3].set_ylabel(i, fontsize=15)
    axs[idx // 3, idx % 3].set_xlabel('Time (day)', fontsize=15)
    axs[idx // 3, idx % 3].tick_params(axis='both', which='major', labelsize=12)
    axs[idx // 3, idx % 3].legend(loc='best', fontsize=15)
    
    # Ensure y-axis range is larger than prediction upper
    max_y = max(data_FDA['Prediction_Upper'][i].max(), data_FDA['Measurement'][i].max())
    axs[idx // 3, idx % 3].set_ylim(0, max_y * 1.1)
...
plt.tight_layout()
plt.savefig('FDA_prediction.png')
plt.show()
