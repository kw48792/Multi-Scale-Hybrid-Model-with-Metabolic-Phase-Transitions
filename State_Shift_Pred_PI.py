...
with open('params_A23.pkl', 'rb') as f:
    theta = pickle.load(f)



#%% Deterministic model prediction with the optimized parameters
batch = 'tigr15_3'  # change or extend this list as needed
l = 5.0  # days ahead
# use positional indexing to get the last Day value and np.arange for float step sizes
last_day = data_extra_cont[batch]['Day'].iloc[-1]
t = np.arange(0, last_day + Delta_t, Delta_t)
t = np.unique(np.concatenate((t, feeding_cont[batch]['Day'].values))) # Ensure feeding days are included
# print(t)

x0 = np.concatenate((np.zeros(target_label_mapper['ANTI'] + 1), data_extra_cont[batch].iloc[0, 1:-1].values))
x0 = np.hstack((x0, np.array((data_extra_cont[batch].iloc[0, -1] * biomass, 0, 0, 0, data_extra_cont[batch].iloc[0, -1], 0, 0, 0))))  # Initial VCD and Biomass
x_pred_VCD = model_prediction_determ(t, x0, theta, mode = 0, batch=batch, t_eval=None, growth_prop=0, metabolite_prop=0)


# container to store full predicted state vector at each measurement time in t
# 'times' holds the timepoints, 'states' will be appended with np.array state vectors
predictions_over_time = {
    'times': [],    # append timepoints for each prediction
    'states': [],   # append full state vectors (np.array) for each prediction
}

# keys to evaluate (use subset similar to calc_wape_for_batch)
extracellular_keys = ['EGLC','EGLN','EGLU','ELAC',
                        'ELEU','EILE','EVAL','EANTI','VCD']

# accumulators for WAPE calculation
abs_errs_by_key = {k: 0.0 for k in extracellular_keys}
obs_abs_by_key = {k: 0.0 for k in extracellular_keys}
valid_counts = {k: 0 for k in extracellular_keys}
total_abs = 0.0
total_obs_abs = 0.0

for day in range(0, int(last_day - l) + 1):
    if day == 0:
        x0 = np.concatenate((np.zeros(target_label_mapper['ANTI'] + 1), data_extra_cont[batch].iloc[0, 1:-1].values))  # use initial condition from data
    else:
        x0 = np.concatenate((np.zeros(target_label_mapper['ANTI'] + 1), data_extra_cont[batch].iloc[2*day-1, 1:-1].values))  # use last predicted state as initial condition
    # find index in t that corresponds to this integer day (use isclose fallback)
    idxs = np.where(np.isclose(t, day))[0]
    if idxs.size:
        ti = idxs[0]
    else:
        ti = int(np.argmin(np.abs(t - day)))

    # pick the corresponding state from the precomputed x_pred_VCD (states x times)
    biom0 = float(x_pred_VCD[target_label_mapper['BIOM_0'], ti])
    biom1 = float(x_pred_VCD[target_label_mapper['BIOM_1'], ti])
    biom2 = float(x_pred_VCD[target_label_mapper['BIOM_2'], ti])
    biom3 = float(x_pred_VCD[target_label_mapper['BIOM_3'], ti])
    vcd0  = float(x_pred_VCD[target_label_mapper['VCD_0'],  ti])
    vcd1  = float(x_pred_VCD[target_label_mapper['VCD_1'],  ti])
    vcd2  = float(x_pred_VCD[target_label_mapper['VCD_2'],  ti])
    vcd3  = float(x_pred_VCD[target_label_mapper['VCD_3'],  ti])

    x0 = np.hstack((x0, np.array((biom0, biom1, biom2, biom3, vcd0, vcd1, vcd2, vcd3))))
    # pick prediction window from t: times between day and day + l (inclusive)
    predict_window = (t >= day) & (t <= day + l)
    predict_t = t[predict_window]
    x_pred = model_prediction_determ(predict_t, x0, theta, mode=0, batch=batch, t_eval=None, growth_prop=0, metabolite_prop=0)
    # store only the last state vector (prediction at horizon day + l)
    predictions_over_time['times'].append(day+l)
    predictions_over_time['states'].append(x_pred[:, -1].copy())

# calculate WAPE per key based on predictions_over_time from day l onward
for i, pred_time in enumerate(predictions_over_time['times']):
    pred_state = predictions_over_time['states'][i]
    # find measurement row(s) at time pred_time (use isclose to avoid float-equality issues)
    mask = np.isclose(data_extra_cont[batch]['Day'].values, pred_time)
    if not np.any(mask):
        continue
    meas_row = data_extra_cont[batch].iloc[np.where(mask)[0][0]]

    for key in extracellular_keys:
        if key == 'VCD':
            pred_val = float(pred_state[target_label_mapper['VCD_0']]
                                + pred_state[target_label_mapper['VCD_1']]
                                + pred_state[target_label_mapper['VCD_2']]
                                + pred_state[target_label_mapper['VCD_3']])
            meas_val = meas_row['VCD_0']
            obs_abs= np.nanmax(data_extra_cont[batch]['VCD_0'])
        else:
            idx = target_label_mapper[key]
            pred_val = float(pred_state[idx])
            meas_val = meas_row[key]
            obs_abs = np.nanmax(data_extra_cont[batch][key])

        # skip if either is NaN
        if pd.isna(meas_val) or pd.isna(pred_val):
            continue

        abs_err = float(np.abs(pred_val - meas_val))

        abs_errs_by_key[key] += abs_err
        obs_abs_by_key[key] += obs_abs
        valid_counts[key] += 1
        total_abs += abs_err
        total_obs_abs += obs_abs

# compute WAPE per key
wape_by_key = {}
for key in extracellular_keys:
    if obs_abs_by_key[key] > 0:
        wape = 100.0 * abs_errs_by_key[key] / obs_abs_by_key[key]
    else:
        wape = np.nan
    wape_by_key[key] = wape
overall_wape = 100.0 * total_abs / total_obs_abs if total_obs_abs > 0 else np.nan
print(f"Batch {batch}  {int(l)}-day-ahead WAPE:")
for key in extracellular_keys:
    print(f"  {key}: {wape_by_key[key]:.2f}%")
